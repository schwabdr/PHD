'''
11/4/22
Reworking this file to work on the MalImg dataset.




Old notes below this line.
...
def MI_loss(model_fns, x_natural, y_true ,x_adv, alpha=5.0, lambd=0.1, iter=0):
    ...
    loss_mea_n = torch.tensor(0.0).cuda()
...
def eval_loss(model_fns, device, test_loader, loss_fn):
    ...
    for eps in eps_lst:
        ...
        #nb_iter = round(eps/eps_iter) + 10
        nb_iter = 100
        print(f"Using MI-Craft with eps: {eps}, eps_iter: {eps_iter}, nb_iter: {nb_iter}")
        #with torch.no_grad():
        i = 0
        for x_natural, y_true in test_loader:
            i = i+1

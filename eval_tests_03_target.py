...
args.batch_size = 512 #512 worked
    ...
    for eps in eps_lst:
        ...
        for x_natural, y_true in test_loader:
            ...
            for y_tar in y_target:
                ...
                while y_tar.item() != y_true[j]:
                    j += 1
                if first:
                    x_target = x_natural[j] #shape is [3,32,32]
                    ...
                    x_target = torch.cat([x_target, x_natural[j].unsqueeze(0)], dim=0)  
                
                #print(f"x_target.shape: {x_target.shape}") last iteration is [batch_size, 3, 32, 32] as desired
